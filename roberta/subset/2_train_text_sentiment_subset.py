...
# Feature columns: Direct comment sentiment features
FEATURE_COLS = [
    # "sentiment_positive",
    # "sentiment_neutral",
    # "sentiment_negative",
...
def main():
    ...
    print(f"Executing File 2 (Text + Sentiment) [SUBSET ONLY] on Device: {device}")
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_metrics["f1"] > best_dev_f1:
            ...
            save_checkpoint(model, "checkpoints/best_model_2.pt")
            print(f"  --> Saved new best model checkpoint (Dev F1: {best_dev_f1:.4f})")

    # Generate and save the visual plot after training finishes
    plot_learning_curves(history, save_path="results/learning_curves_subset2.png")    
    ...
    test_metrics["experiment"] = "2_text_sentiment_subset"
    
    print("\n--- Final Test Set Results (Subset) ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results2_subset.csv")
    save_checkpoint(model, "checkpoints/2_text_sentiment_subset.pt")
