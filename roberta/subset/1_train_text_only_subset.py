...
# Feature columns: None (Text Only)
FEATURE_COLS = []
...
def main():
    ...
    print(f"Executing File 1 (Text Only) [SUBSET ONLY] on Device: {device}")
    ...
    # 2. Numerical Features (None for text-only baseline)
    ...
    plot_learning_curves(history, save_path="results/learning_curves_subset1.png")    
    ...
    test_metrics["experiment"] = "1_text_only_subset"
    
    print("\n--- Final Test Set Results (Subset) ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results1_subset.csv")
    save_checkpoint(model, "checkpoints/1_text_only_subset.pt")
