...
# Feature columns: Sentiment and delay combined
FEATURE_COLS = [
    # "sentiment_positive",
    # "sentiment_neutral",
    # "sentiment_negative",
...
def main():
    ...
    print(f"Executing File 5 (Text + Sentiment + Delay) [SUBSET ONLY] on Device: {device}")
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_metrics["f1"] > best_dev_f1:
            ...
            save_checkpoint(model, "checkpoints/best_model_5.pt")
            print(f"  --> Saved new best model checkpoint (Dev F1: {best_dev_f1:.4f})")

    # Generate and save the visual plot after training finishes
    plot_learning_curves(history, save_path="results/learning_curves_subset5.png")    
    ...
    test_metrics["experiment"] = "5_text_sentiment_delay_subset"
    
    print("\n--- Final Test Set Results (Subset) ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results5_subset.csv")
    save_checkpoint(model, "checkpoints/5_text_sentiment_delay_subset.pt")
