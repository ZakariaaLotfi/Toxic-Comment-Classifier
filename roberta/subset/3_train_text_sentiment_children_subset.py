...
# Feature columns: Sentiment + Children features
FEATURE_COLS = [
    # "sentiment_positive",
    # "sentiment_neutral",
    # "sentiment_negative",
...
def main():
    ...
    print(f"Executing File 3 (Text + Sentiment + Children) [SUBSET ONLY] on Device: {device}")
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_metrics["f1"] > best_dev_f1:
            ...
            save_checkpoint(model, "checkpoints/best_model_3.pt")
            print(f"  --> Saved new best model checkpoint (Dev F1: {best_dev_f1:.4f})")

    # Generate and save the visual plot after training finishes
    plot_learning_curves(history, save_path="results/learning_curves_subset3.png")    
    
    # 5. Optimize Threshold on Dev Split (Strategy 2)
    ...
    # 6. Final Test Evaluation (Strategy 4)
    ...
    test_metrics["experiment"] = "3_text_sentiment_children_subset"
    
    print("\n--- Final Test Set Results (Subset) ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results3_subset.csv")
    save_checkpoint(model, "checkpoints/3_text_sentiment_children_subset.pt")
