...
def main():
    ...
    print(f"Executing File 7 (All Features Combined) [SUBSET ONLY] on Device: {device}")
    ...
    plot_learning_curves(history, save_path="results/learning_curves_subset7.png")    
    ...
    test_metrics["experiment"] = "7_all_features_subset"
    
    print("\n--- Final Test Set Results (Subset) ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results7_subset.csv")
    save_checkpoint(model, "checkpoints/7_all_features_subset.pt")
