...
# Configuration
CSV_PATH = "../../data_with_depths.csv"
MODEL_PATH = "../../models/models--roberta-base/snapshots/e2da8e2f811d1448a5b465c236feacd80ffbac7b"
BATCH_SIZE = 64
EPOCHS = 8
LR = 2e-5
FEATURE_COLS = []
USE_SAMPLER = False  # Set to True if using WeightedRandomSampler instead of pos_weight in loss


def find_optimal_threshold(dev_probs, dev_labels):
    """Strategy 2: Searches decision thresholds between 0.10 and 0.90 to maximize Dev F1 score."""
...
def main():
    ...
    print(f"Executing File 1 (Text Only) on Device: {device}")
    
    # 1. Load Data with Official Splits (Train, Dev, Test)
    ...
    train_dataset = ContextualToxicityDataset(train_df["meta_text"], train_df["toxic"], None, tokenizer)
    dev_dataset = ContextualToxicityDataset(dev_df["meta_text"], dev_df["toxic"], None, tokenizer)
    test_dataset = ContextualToxicityDataset(test_df["meta_text"], test_df["toxic"], None, tokenizer)
    ...
    # pos_weight for Loss Function (~4.41)
    pos_weight = torch.tensor([1.0] if USE_SAMPLER else [num_neg / num_pos], device=device)
    criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    
    model = RobertaContextualClassifier(model_path_or_name=MODEL_PATH, num_extra_features=0).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        if dev_metrics["f1"] > best_dev_f1:
            ...
            save_checkpoint(model, "checkpoints/best_model_1.pt")
            print(f"  --> Saved new best model checkpoint (Dev F1: {best_dev_f1:.4f})")

    # Generate and save the visual plot after training finishes
    plot_learning_curves(history, save_path="results/learning_curves_full_data1.png")    
    
    # Optimize Threshold on Dev Split
    ...
    if isinstance(dev_results, dict) and "probs" in dev_results:
        dev_probs = dev_results["probs"]
        optimal_thresh, best_dev_f1 = find_optimal_threshold(dev_probs, dev_df["toxic"].values)
    else:
        optimal_thresh = 0.5  # Default fallback if evaluate function returns summary metrics only
        best_dev_f1 = dev_metrics.get("f1", 0.0)

    print(f"\nOptimal Dev Decision Threshold: {optimal_thresh:.2f} (Dev Binary F1: {best_dev_f1:.4f})")

    # Final Test Evaluation using Optimal Threshold & Comprehensive Metrics
    ...
    test_metrics["experiment"] = "1_text_only"
    ...
    save_results(test_metrics, "results/experiment_results1.csv")
    save_checkpoint(model, "checkpoints/1_text_only.pt")
