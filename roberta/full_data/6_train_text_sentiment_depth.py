...
def main():
    ...
    print(f"Executing File 6 (Text + Sentiment + Depth) on Device: {device}")
    ...
    plot_learning_curves(history, save_path="results/learning_curves_full_data6.png")    
    ...
    test_metrics["experiment"] = "6_text_sentiment_depth"
    
    print("\n--- Final Test Set Results ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results6.csv")
    save_checkpoint(model, "checkpoints/6_text_sentiment_depth.pt")
