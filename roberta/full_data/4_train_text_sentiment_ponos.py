...
# Text + sentiment + PONOS network structural features
FEATURE_COLS = [
    # "sentiment_positive",
    # "sentiment_neutral",
    # "sentiment_negative",
...
def main():
    ...
    print(f"Executing File 4 (Text + Sentiment + PONOS) on Device: {device}")
    ...
    plot_learning_curves(history, save_path="results/learning_curves_full_data4.png")    
    ...
    test_metrics["experiment"] = "4_text_sentiment_ponos"
    
    print("\n--- Final Test Set Results ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results4.csv")
    save_checkpoint(model, "checkpoints/4_text_sentiment_ponos.pt")
