...
# Text + sentiment + child comments sentiment context
FEATURE_COLS = [
    # "sentiment_positive",
    # "sentiment_neutral",
    # "sentiment_negative",
...
def main():
    ...
    print(f"Executing File 3 (Text + Sentiment + Children) on Device: {device}")
    ...
    plot_learning_curves(history, save_path="results/learning_curves_full_data3.png")    
    ...
    test_metrics["experiment"] = "3_text_sentiment_children"
    
    print("\n--- Final Test Set Results ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results3.csv")
    save_checkpoint(model, "checkpoints/3_text_sentiment_children.pt")
