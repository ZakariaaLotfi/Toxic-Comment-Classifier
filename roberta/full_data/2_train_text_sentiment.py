...
def main():
    ...
    print(f"Executing File 2 (Text + Sentiment) on Device: {device}")
    
    # 1. Load Data with Official Splits
    ...
    plot_learning_curves(history, save_path="results/learning_curves_full_data2.png")    
    ...
    test_metrics["experiment"] = "2_text_sentiment"
    
    print("\n--- Final Test Set Results ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results2.csv")
    save_checkpoint(model, "checkpoints/2_text_sentiment.pt")
