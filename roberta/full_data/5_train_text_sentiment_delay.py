...
# All available contextual features combined
FEATURE_COLS = [
    # "sentiment_positive",
    # "sentiment_neutral",
    # "sentiment_negative",
...
def main():
    ...
    print(f"Executing File 5 (Text + Sentiment + Delay) on Device: {device}")
    ...
    plot_learning_curves(history, save_path="results/learning_curves_full_data5.png")    
    ...
    test_metrics["experiment"] = "5_text_sentiment_delay"
    
    print("\n--- Final Test Set Results ---")
    for k, v in test_metrics.items():
        print(f"  {k}: {v:.4f}" if isinstance(v, float) else f"  {k}: {v}")
            
    save_results(test_metrics, "results/experiment_results5.csv")
    save_checkpoint(model, "checkpoints/5_text_sentiment_delay.pt")
